...
renderCUDA(
	...
	const float* __restrict__ language_feature,  //language_feature
	...
	float* __restrict__ dL_dlanguage_feature)
{
	...
	for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
	{
		...
		if (range.x + progress < range.y)
		{
			...
			for (int i = 0; i < F; i++)
				collected_features[i * BLOCK_SIZE + block.thread_rank()] = language_feature[coll_id * F + i];
		}
		block.sync();
		...
		for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
		{
			...
			for (int ch = 0; ch < F; ch++)
			{
				...
				// Update last language features (to be used in the next iteration)
...
void BACKWARD::render(
	...
	const float* colors,
	const float* language_feature, //language_feature
	...
	float* dL_dlanguage_feature)
{
	...
	// printf( "--------Collapsed time for Rendering(cuda) backward： %f ms\n", duration );  
}